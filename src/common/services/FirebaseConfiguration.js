...
import { login as loginHandle, logout as logoutHandle } from "../store/reducers/auth";
...
export const emailVerification = async () => {
	try {
		...
		toast.success(`Verification mail has been sent to ${auth.currentUser.email}`);
	} catch (error) {
		toast.error(error.message);
	}
};
...
onSnapshot(collection(db, "posts"), doc => {
	store.dispatch(setPosts(doc.docs.reduce((posts, post) => [...posts, post.data()], [])));
});
...
export const getUser = async email => {
	try {
		...
		const q = query(usersdb, where("mail", "==", email));
